...
function counselorManagement() {
    ...
    function setupEventListeners() {
		...
		btnHistoryReqTime && btnHistoryReqTime.addEventListener('click', ()=> activeSortHistory(btnHistoryReqTime, "crRate"));
        ...
        btnModify && btnModify.addEventListener('click', async () => {
            ...
            if (!memId) { alert('대상이 없습니다.'); return; }
        ...
        // 차트 기간 셀렉트(기간 선택만 정상동작하게 처리)
        cateDateSel  && cateDateSel.addEventListener('change', eventDateRangeSelect);
		typeDateSel  && typeDateSel.addEventListener('change', eventDateRangeSelect);
		top3DateSel  && top3DateSel.addEventListener('change', eventDateRangeSelect);
		hoursDateSel && hoursDateSel.addEventListener('change', eventDateRangeSelect);
    }
    ...
    async function loadDetail() {
        ...
        try {
            ...
            recent1El && (recent1El.innerHTML = `<i class="fa-regular fa-clock"></i>${fmtYMD(data.recentCounselHistory)}`);
            recent2El && (recent2El.innerHTML = `<i class="fa-regular fa-clock"></i>${fmtYMD(data.recentVacationHistory)}`);
            ...
            try {
                ...
                // 우선순위: UMG값 → 없으면 CSMG값
                if (recentLg) { recent1El && (recent1El.innerHTML = `<i class="fa-regular fa-clock"></i>${fmtYMD(recentLg)}`); }
                if (recentPn) { recent2El && (recent2El.innerHTML = `<i class="fa-regular fa-clock"></i>${fmtYMD(recentPn)}`); }
            } catch (e) {
                // optional
            }
        } catch (err) {
                ...
                if (!data.content || data.content.length === 0) {
                    ...
                    tbodyHistory.innerHTML = data.content.map(r => `
                        <tr>
                            ...
                            <td>${fmtYMD(r.COUNSEL_REQ_DATETIME)}</td>
	...
	function counselingStatsByTimeChartCanvas() {
		...
		// 시리즈별 스타일(붉은계열 피하고, 보라/그린/블루 사용)
		const SERIES_STYLES = {
			'대면상담': { border: '#6C4DDC', point: '#6C4DDC', gradFrom: 'rgba(108,77,220,0.25)', gradTo: 'rgba(108,77,220,0.00)' },
			'채팅상담': { border: '#00C896', point: '#00C896', gradFrom: 'rgba(0,200,150,0.20)', gradTo: 'rgba(0,200,150,0.00)' },
			'화상상담': { border: '#2DA3FF', point: '#2DA3FF', gradFrom: 'rgba(45,163,255,0.20)', gradTo: 'rgba(45,163,255,0.00)' }
		};
