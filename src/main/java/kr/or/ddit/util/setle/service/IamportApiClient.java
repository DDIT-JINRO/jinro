...
public class IamportApiClient {
	...
	public String getAccessToken() throws JsonProcessingException {
		...
		try {
			...
			/*
			 * "response"가 변수 이름이 아니라, API 응답 JSON의 특정 '키(key)'를 나타내는 문자열이기 때문에 ""로 담는다 
			 * 성공응답 예시 
			 * { 
			 * 		"code": 0, 
			 * 		"message": "성공", 
			 * 		"response": { // <-- 여기가 "response"라는 키(key)입니다. 
			 * 			"access_token": "발급된_토큰_값", 
			 * 			"now": 1678886400, 
	...
	/**
	 * imp_uid(아임포트 결제 고유 ID)를 사용하여 특정 결제 정보를 조회하는 메서드입니다. 
	 * 클라이언트로부터 전달받은 결제 정보를 서버에서 검증할 때 사용됩니다.
	 *
    ...
    /**
     * customer_uid(고객 고유 식별자)를 사용하여 정기 결제를 예약하는 메서드입니다.
     * 일반적으로 최초 빌링키 발급 후 다음 자동 결제를 미리 예약할 때 사용됩니다.
     * 아임포트 API: POST /subscribe/payments/schedule
     *
     ...
     * 아임포트 API: POST /subscribe/payments/again
     *
