...
public class GenerateTextFromTextInput {
    ...
    public String geminiChat(@RequestBody Map<String, String> message) {
        ...
        String selectedSysPrompt = null;
        if("MIND".equals(topic)) {
        	selectedSysPrompt = systemPrompt1;
        }else if("JOB".equals(topic)) {
        	selectedSysPrompt = systemPrompt2;
        }else if("STUDY".equals(topic)) {
        	selectedSysPrompt = systemPrompt3;
        }else {
        	throw new CustomException(ErrorCode.INVALID_INPUT);
        }

        String prompt = selectedSysPrompt + "\n" + message.get("message");
