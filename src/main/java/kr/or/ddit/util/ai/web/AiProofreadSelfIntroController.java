package kr.or.ddit.util.ai.service;
...
public class AiProofreadService {
    ...
    // API 키는 애플리케이션 외부 설정 파일 (예: src/main/resources/application.properties 또는 application.yml)에 저장하고 @Value로 주입받는 것이 좋습니다.
    // 예: google.gemini.api-key=YOUR_API_KEY_HERE
    // @Value("${google.gemini.api-key}") // 실제 환경에서는 이 어노테이션을 사용합니다.
    private String apiKey = "YOUR_NEW_API_KEY_HERE"; // 개발/테스트용으로 여기에 직접 입력해도 됩니다.
    ...
    private static final String SELF_INTRO_PROOFREAD_PROMPT = """
        ...
        8.  **글자 수 제한 준수:** 각 섹션별로 제공된 '현재 글자 수'와 '목표 글자 수'를 고려하여 피드백과 수정 제안을 제공하세요. 만약 수정 제안이 목표 글자 수를 초과하거나 현저히 부족하다면, 이에 대한 조언을 피드백에 포함하세요.
          ...
          "sections_feedback": [
            {
              ...
              "current_char_count": "[해당 섹션의 현재 글자 수]",
              "target_char_limit": "[해당 섹션의 목표 글자 수]",
              "feedback_items": [
                {
        ...
        [질문 제목] (현재 글자 수: [현재 글자 수]자 / 목표 글자 수: [목표 글자 수]자)
        [답변 내용]
        ---
        다음 질문 내용:
        [질문 제목] (현재 글자 수: [현재 글자 수]자 / 목표 글자 수: [목표 글자 수]자)
    ...
    /**
     * 자기소개서 첨삭을 요청하고 AI 응답 JSON 문자열을 반환합니다.
     * 이 메서드는 오직 AI 호출 로직만 포함하며, 데이터 조회는 관여하지 않습니다.
     *
    ...
    public String proofreadCoverLetter(List<Map<String, String>> selfIntroSections) {
        ...
        String formattedSectionsForPrompt = selfIntroSections.stream()
            .map(section -> {
                String questionTitle = section.getOrDefault("questionTitle", "제목 없음");
                String content = section.getOrDefault("content", "");
                String currentCharCount = section.getOrDefault("currentCharCount", "0");
                String targetCharLimit = section.getOrDefault("targetCharLimit", "미정");
                return String.format("%s (현재 글자 수: %s자 / 목표 글자 수: %s자)\n%s",
                                     questionTitle, currentCharCount, targetCharLimit, content);
            })
        ...
        try {
            ...
            // Gemini 모델에 텍스트 생성 요청 (자기소개서와 같이 긴 문서 처리에 적합한 모델 사용)
