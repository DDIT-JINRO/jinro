									...
									<div class="com-filter-options">
										<c:forEach var="cType" items="${regionList}">
											<label class="com-filter-item"> <input
												type="checkbox" name="regionFilter" value="${cType.ccId}"
												data-label="지역" data-name="${cType.ccEtc}"
												<c:if test="${fn:contains(checkedFilters.regionFilter, cType.ccId)}">checked</c:if> />
								...
								<div class="com-filter-section">
									...
									<div class="com-filter-options">
										<c:forEach var="cType" items="${schoolTypeList}">
											<label class="com-filter-item"> <input
												type="checkbox" name="schoolType" value="${cType.ccId}"
												data-label="학교 유형" data-name="${cType.ccName}"
												<c:if test="${fn:contains(checkedFilters.schoolType, cType.ccId)}">checked</c:if> />
												<span>${cType.ccName}</span>
											</label>
										</c:forEach>
									</div>
								...
								<div class="com-filter-section">
									...
									<div class="com-filter-options">
										<c:forEach var="cTarget" items="${coedTypeList}">
											<label class="com-filter-item"> <input
												type="checkbox" name="coedTypeFilter"
												value="${cTarget.ccId}" data-label="공학 여부"
												data-name="${cTarget.ccName}"
												<c:if test="${fn:contains(checkedFilters.coedTypeFilter, cTarget.ccId)}">checked</c:if> />
				...
				<ul class="pagination">
					<li>
						<%-- c:url 태그로 이전 페이지 링크 생성 --%> <c:url
							value="/ertds/hgschl/selectHgschList.do" var="prevUrl">
							...
							<c:forEach var="filter" items="${checkedFilters.regionFilter}">
								<c:param name="regionFilter" value="${filter}" />
							</c:forEach>
							<c:forEach var="filter" items="${checkedFilters.schoolType}">
								<c:param name="schoolType" value="${filter}" />
							</c:forEach>
							<c:forEach var="filter" items="${checkedFilters.coedTypeFilter}">
								<c:param name="coedTypeFilter" value="${filter}" />
							</c:forEach>
					...
					<c:forEach var="pNo" begin="${articlePage.startPage}"
						...
						<li>
							<%-- c:url 태그로 각 페이지 번호 링크 생성 --%> <c:url
								value="/ertds/hgschl/selectHgschList.do" var="pageUrl">
					...
					<li>
						<%-- c:url 태그로 다음 페이지 링크 생성 --%> <c:url
							value="/ertds/hgschl/selectHgschList.do" var="nextUrl">
							<c:param name="currentPage" value="${articlePage.startPage + 5}" />
							<c:param name="keyword" value="${checkedFilters.keyword}" />
							<%-- 현재 적용된 모든 필터 값들을 다시 파라미터로 추가 --%>
							<c:forEach var="filter" items="${checkedFilters.regionFilter}">
