	...
	<div class="public-wrapper">
		...
		<div class="public-wrapper-main">
			...
			<c:forEach var="stdBoardVO" varStatus="stat" items="${articlePage.content }">
				<div class="group-card" data-stdb-id="${stdBoardVO.boardId }">
					...
					<div class="group-meta">
						👁️ ${stdBoardVO.boardCnt} &nbsp; 💬 ${stdBoardVO.replyCnt} &nbsp;&nbsp;
			...
			<div class="card-footer clearfix">
				<ul class="pagination">
					<li>
						<a href="${articlePage.url }&currentPage=${articlePage.startPage - 5}" class="<c:if test='${articlePage.startPage < 6}'>disabled</c:if>"> ← Previous </a>
					</li>

					<c:forEach var="pNo" begin="${articlePage.startPage}" end="${articlePage.endPage}">
						<li>
							<a href="${articlePage.url }&currentPage=${pNo}" class="<c:if test='${pNo == articlePage.currentPage}'>active</c:if>"> ${pNo} </a>
						</li>
					</c:forEach>

					<li>
						<a href="${articlePage.url }&currentPage=${articlePage.startPage + 5}" class="<c:if test='${articlePage.endPage >= articlePage.totalPages}'>disabled</c:if>"> Next → </a>
					</li>
				</ul>
			</div>
		</div>
