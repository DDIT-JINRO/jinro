...
<link rel="stylesheet" href="/css/prg/act/sup/supList.css">
<!-- 스타일 여기 적어주시면 가능 -->
	...
	<div class="public-wrapper">
		...
		<div class="public-wrapper-main">
			<div class="filter-section">
				<form method="get" action="/prg/act/sup/supList.do">
								...
								<div class="com-filter-section">
									...
									<div class="com-filter-options">
										<label class="com-filter-item"> <input type="checkbox"
											name="contestStatusFilter" value="proceeding"
											data-label="모집 상태" data-name="진행중"
											<c:if test="${fn:contains(checkedFilters.contestStatusFilter, 'proceeding')}">checked</c:if> />
											<span>진행중</span>
										</label> <label class="com-filter-item"> <input
											type="checkbox" name="contestStatusFilter" value="finished"
											data-label="모집 상태" data-name="마감"
								...
								<div class="com-filter-section">
									...
									<div class="com-filter-options">
										<c:forEach var="cTarget" items="${contestTargetList}">
											<label class="com-filter-item"> <input
												type="checkbox" name="contestTargetFilter"
												value="${cTarget.ccId}" data-label="모집 대상"
												data-name="${cTarget.ccName}"
			...
			<div class="list-container">
				<c:if test="${not empty articlePage.content}">
					<c:forEach var="contest" items="${articlePage.content}">
						<a href="/prg/act/sup/supDetail.do?supId=${contest.contestId}"
							class="contest-card">
							<div class="card-image-box">
								<img
									src="/files/download?fileGroupId=${contest.fileGroupId}&seq=1"
							...
							<div class="card-content">
								...
								<div class="contest-description">
									<ul class="card-info-list">
										<li><span class="info-label">주최</span> <span
											class="info-value">${contest.contestHost}</span></li>
										<li><span class="info-label">접수기간</span> <span
											class="info-value"> <fmt:formatDate
													value="${contest.contestStartDate}" pattern="yyyy.MM.dd" />
												~ <fmt:formatDate value="${contest.contestEndDate}"
													pattern="yyyy.MM.dd" />
										</span></li>
										<li class="d-day-item"
											data-end-date="<fmt:formatDate value='${contest.contestEndDate}' pattern='yyyy-MM-dd' />">
											<span class="info-label">마감까지</span> <span
											class="info-value d-day-text"> <%-- JavaScript가 여기에 D-Day를 채웁니다. --%>
										</span>
										</li>
										<li><span class="info-label">모집상태</span> <span
											class="info-value"> <%-- 현재 날짜와 마감 날짜를 비교하여 진행/마감 표시 --%>
												<fmt:formatDate value="${contest.contestEndDate}"
													pattern="yyyyMMdd" var="endDay" /> <fmt:formatDate
													value="${now}" pattern="yyyyMMdd" var="today" /> <c:choose>
								...
								<div class="contest-meta">
									<span class="meta-item"> ${contest.contestGubunName} |
										${contest.contestTypeName} | ${contest.contestTargetName} </span> <br />
									<span class="meta-item">조회수
										${contest.contestRecruitCount}</span> <span class="meta-item">
										<fmt:formatDate value="${contest.contestCreatedAt}"
											pattern="yyyy. MM. dd" />
									</span>
								</div>
							</div>
			...
			<ul class="pagination">
				<li>
					<%-- c:url 태그로 이전 페이지 링크 생성 --%> <c:url value="/prg/act/sup/supList.do"
						...
						<c:forEach var="filter"
							items="${checkedFilters.contestTargetFilter}">
							<c:param name="contestTargetFilter" value="${filter}" />
						</c:forEach>
						<c:forEach var="filter"
							items="${checkedFilters.contestStatusFilter}">
				...
				<c:forEach var="pNo" begin="${articlePage.startPage}"
					...
					<li>
						<%-- c:url 태그로 각 페이지 번호 링크 생성 --%> <c:url
							value="/prg/act/sup/supList.do" var="pageUrl">
						...
						class="${pNo == articlePage.currentPage ? 'active' : ''}">${pNo}</a>
					</li>
				</c:forEach>

				<li>
					<%-- c:url 태그로 다음 페이지 링크 생성 --%> <c:url value="/prg/act/sup/supList.do"
						var="nextUrl">
						<c:param name="currentPage" value="${articlePage.startPage + 5}" />
						<c:param name="keyword" value="${checkedFilters.keyword}" />
						<%-- 현재 적용된 모든 필터 값들을 다시 파라미터로 추가 --%>
					...
					class="${articlePage.endPage >= articlePage.totalPages ? 'disabled' : ''}">Next
						→</a>
				</li>
			</ul>
		</div>
