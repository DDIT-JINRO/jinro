<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ include file="/WEB-INF/views/include/header.jsp"%>
<link rel="stylesheet" href="">
<!-- 스타일 여기 적어주시면 가능 -->
<section class="channel">
	<!-- 	여기가 네비게이션 역할을 합니다.  -->
	<div class="channel-title">
		<!-- 대분류 -->
		<div class="channel-title-text">커뮤니티</div> 
	</div>
	<div class="channel-sub-sections">
		<!-- 중분류 -->
		<div class="channel-sub-section-itemIn"><a href="/comm/peer/teen/teenList.do">또래 게시판</a></div> <!-- 중분류 -->
		<div class="channel-sub-section-item"><a href="/comm/path/pathList.do">진로/진학 게시판</a></div>
	</div>
</section>
<div>
	<div class="public-wrapper">
		<!-- 여기는 소분류(tab이라 명칭지음)인데 사용안하는곳은 주석처리 하면됩니다 -->
		<div class="tab-container" id="tabs">
		    <div class="tab"><a href="/comm/peer/teen/teenList.do">청소년 게시판</a></div>
		    <div class="tab active"><a href="/comm/peer/youth/youthList.do">청년 게시판</a></div>
  		</div>
		<!-- 여기부터 작성해 주시면 됩니다 -->
  		<div class="public-wrapper-main">
  			청소년 게시판 목록 페이지
  			<br/>
  			<a href="/comm/peer/youth/youthDetail.do?boardId=1">청년 1번 게시글</a>
  			<br/>
  			<a href="/comm/peer/youth/youthDetail.do?boardId=2">청년 2번 게시글</a>
  			<br/>
  			<a href="/comm/peer/youth/youthDetail.do?boardId=3">청년 3번 게시글</a>
  			<br/><br/><br/><br/>
  			<a href="/comm/peer/youth/youthInsert.do">청년 게시글 작성</a>
  		</div>
	</div>
</div>
<%@ include file="/WEB-INF/views/include/footer.jsp"%>
</body>
</html>
<script>
	// 스크립트 작성 해주시면 됩니다.
</script>