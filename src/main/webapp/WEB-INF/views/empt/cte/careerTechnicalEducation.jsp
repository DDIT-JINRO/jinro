	...
	<div class="public-wrapper">
		...
		<div class="public-wrapper-main">
				...
				<div class="search-filter__accordion">
					...
					<div class="search-filter__accordion-panel" id="search-filter-panel">
						<div class="search-filter__accordion-content">
							...
							<div class="search-filter__group">
								<label class="search-filter__group-title">마감순</label>
								<div class="search-filter__options">
									<label class="search-filter__option">
										<input type="checkbox" name="status" value="마감빠른순">
										<span>마감 빠른 순</span>
									</label>
									<label class="search-filter__option">
										<input type="checkbox" name="status" value="마감느린순">
										<span>마감 느린 순</span>
									</label>
								</div>
							</div>
			...
			<div class="pagination">
				<a href="${articlePage.url}?currentPage=${articlePage.startPage - 5 > 0 ? articlePage.startPage - 5 : 1}&keyword=${param.keyword}&status=${param.status}&region=${param.region}" class="pagination__link ${articlePage.startPage < 6 ? 'pagination__link--disabled' : ''}"> ← Previous </a>

				<c:forEach var="pNo" begin="${articlePage.startPage}" end="${articlePage.endPage}">
					<a href="${articlePage.url}?currentPage=${pNo}&keyword=${param.keyword}&status=${param.status}&region=${param.region}" class="pagination__link ${pNo == articlePage.currentPage ? 'pagination__link--active' : ''}"> ${pNo} </a>
				</c:forEach>

				<a href="${articlePage.url}?currentPage=${articlePage.endPage + 1 <= articlePage.totalPages ? articlePage.endPage + 1 : articlePage.totalPages}&keyword=${param.keyword}&status=${param.status}&region=${param.region}" class="pagination__link ${articlePage.endPage >= articlePage.totalPages ? 'pagination__link--disabled' : ''}"> Next → </a>
			</div>
		</div>
	</div>
</div>
